...
<style>
    ...
    .lv-card { background: #151923; border: 1px solid #2D313E; border-radius: 16px; padding: 20px; margin-bottom: 12px; position: relative; transition: transform 0.2s;}
    ...
    .streamlit-expanderHeader { background-color: #151923; border-radius: 12px; border: 1px solid #2D313E; color: #FBBF24; font-weight: 800;}
</style>
...
if 'event_pax' not in st.session_state:
    st.session_state.event_pax = "~3000"
...
@st.cache_data(ttl=60) # Junat päivittyy taustalla minuutin välein
                ...
                if timetable[-1].get('stationShortCode') == 'HKI':
                    for row in timetable:
                        if row.get('stationShortCode') == station_code and row.get('type') == 'ARRIVAL':
                            ...
                            delay_mins = 0
                            target_time = scheduled_hel
                            
                            if actual_time: # Juna on jo tullut tälle asemalle
                                continue 
                            elif live_estimate: # Juna on myöhässä/ajoissa
                                est_utc = datetime.datetime.fromisoformat(live_estimate.replace('Z', '+00:00'))
                                target_time = est_utc.astimezone(HELSINKI_TZ)
                                delay_mins = int((target_time - scheduled_hel).total_seconds() / 60)
                            
                            # Näytetään junat jotka ovat tulossa (tai menneet ohi alle 5 min sitten)
                            if target_time > now - datetime.timedelta(minutes=5):
...
@st.cache_data(ttl=120) # Laivat 2 min välein
            ...
            for tr in soup.find_all('tr'):
                ...
                if len(tds) >= 4:
                    ...
                    if ship_name in ship_database:
                        ...
                        # --- AIKASUODATUS LISÄTTY TÄHÄN ---
                        try:
                            h, m = map(int, only_time.split(':'))
                            ship_dt = now.replace(hour=h, minute=m, second=0, microsecond=0)
                            
                            # Jos laiva on saapunut yli 60 minuuttia sitten, ohitetaan se
                            if ship_dt < now - datetime.timedelta(minutes=60):
                                continue
                        except:
                            pass # Jos ajan muunnos epäonnistuu, näytetään laiva varmuuden vuoksi
...
if live_ships:
    ...
    st.info("Ei tulevia laivoja lähitunteina.")
    ...
    for t in trains:
        ...
        <div class="lv-card item-card">
            ...
            <div class="item-details">
                ...
                <div class="item-sub">{sub_text} • Saapu {asemien_nimet[st.session_state.selected_station]}</div>
            </div>
...
# Tapahtuman kortti
st.markdown(f"""
<div class="lv-card" style="margin-bottom: 5px;">
    ...
    <div style="background: rgba(148, 163, 184, 0.1); padding: 4px 8px; border-radius: 4px; display: inline-block; font-size: 0.9rem; color: #94A3B8; margin-bottom: 5px; z-index: 2;">SUURI TAPAHTUMA ({st.session_state.event_pax} hlö)</div>
</div>
""", unsafe_allow_html=True)

# "Kynä"-nappi toteutettuna Expanderilla
with st.expander("✏️ DISPATCH OVERRIDE (Muokkaa tietoja)"):
    uusi_pax = st.text_input("Väkimäärä (Arvio)", value=st.session_state.event_pax)
    c1, c2, c3 = st.columns(3)
    with c1: st.button("✓ OHI", use_container_width=True, key="btn_ohi")
    with c2: st.button("➖ NORMAALI", use_container_width=True, type="primary", key="btn_norm")
    with c3: st.button("⚠️ JONO!", use_container_width=True, key="btn_jono")
    
    if st.button("TALLENNA MUUTOKSET", use_container_width=True):
        st.session_state.event_pax = uusi_pax
        st.rerun()
